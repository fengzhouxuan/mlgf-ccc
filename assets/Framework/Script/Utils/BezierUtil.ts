//来源 https://gitee.com/-/ide/project/muzzik/MKFramework/edit/main/-/assets/tool/tool_bezier_curve.ts

import { Vec3 } from "cc";

export class BezierUtil{
    constructor(point_as_: Vec3[]) {
		...
		/** 临时变量 */
		let temp_v3: Vec3, temp2_v3: Vec3, temp3_v3: Vec3, temp4_v3: Vec3;
	...
	/** 匀速点 */
	uniform_point(pos_n_: number): Vec3 | null {
		if (this.point_v3s.length < 2) {
			console.error("控制点不能少于2");
	...
	private _curve(
		...
		point3_v3_: Vec3,
		point4_v3_: Vec3
	): (((x_n: number) => Vec3) | ((x_n: number) => number))[] {
		// 基本样条线插值算法
		// 弹性
		const s_n = 0.5;
